...
***This data was from an observational study, for there was no assigned treatment groups for people to interfere.***
...
***I expect smokers are more likely to die than nonsmokers.***
...
ggplot(data = Whickham, aes(y = outcome, fill = smoker)) +
  ...
  ggtitle("Distribution of Outcome Based on Smoker")
```
...
```
***This does not meet my expectations, for smokers are more likely to live 20 years later.***
...
ggplot(data = Whickham, aes(y = outcome, fill = smoker)) +
  geom_bar(position = "fill") +
  ggtitle("Distribution of Smoker and Outcome Based on Age") +
  facet_wrap(~age_cat)
```
***In the plot, it seems that smokers are more likely to die in each age category.***
